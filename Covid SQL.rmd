        ...
        FROM (SELECT /* Allocate Unknowns to Natural/Unnatural in proportion to business as a whole, at gender/age_band granularity */
                ...
                FROM (SELECT clm.company_code
                        ...
                        FROM ("assa-lake".v1_covid_claims_25                                  clm INNER JOIN dib_to_use_25_clm dib1 ON     clm.data_import_batch = dib1.max_batch
                                                                                                                                       AND clm.year_month_of_file = dib1.year_month_of_file)
                             LEFT JOIN
             ...
             INNER JOIN (  SELECT policy_number, life_number, MAX (year_month_of_file) max_file
                             FROM "assa-lake".v1_covid_inforce_25
                         GROUP BY policy_number, life_number) fil
                 ON dat.policy_number = fil.policy_number AND dat.life_number = fil.life_number AND dat.year_month_of_file = fil.max_file
    GROUP BY company_code
...
WITH
    ...
    AS
        (SELECT /* Split by company code and use analytic min/max to only use months available for all cos */
                ...
                sequence (MIN (date_trunc ('month', year_and_month_of_date_of_death)), --MAX (MIN (date_trunc ('month', year_and_month_of_date_of_death))) OVER (PARTITION BY NULL),
                          MAX (date_trunc ('month', year_and_month_of_date_of_death)), --MIN (MAX (date_trunc ('month', year_and_month_of_date_of_death))) OVER (PARTITION BY NULL),
    ...
    AS
        (SELECT cur_year.company_code,
                ...
                (1.0 / 12.0) * CASE WHEN next_year.effective_date IS NULL THEN 
                    cur_year.number_of_policies
    ...
    UNION ALL
      SELECT claims.company_code,
        ...
        FROM assa_sandbox.covid_claims AS claims;
    ...
    WITH
        ...
        AS
            (SELECT DISTINCT clm_month
              ...
              WHERE clm_month >= DATE '2020-03-01'),
        covid_groups
        AS
            ...
            (  SELECT company_code
                 ...
                 FROM (  SELECT company_code
                               ...
                               ,SUM (SUM (total_deaths))
                                    OVER (PARTITION BY company_code, sex, age_band ORDER BY clm_month ROWS BETWEEN 2 PRECEDING AND 2 FOLLOWING)    smoothed_total_deaths
                               ,SUM (unnatural_deaths)                                                                                             unnatural_deaths
                               ,SUM (SUM (unnatural_deaths))
                                    OVER (PARTITION BY company_code, sex, age_band ORDER BY clm_month ROWS BETWEEN 2 PRECEDING AND 2 FOLLOWING)    smoothed_unnatural_deaths
                               ,SUM (life_years)                                                                                                   life_years
                               ,SUM (SUM (life_years))
                                    OVER (PARTITION BY company_code, sex, age_band ORDER BY clm_month ROWS BETWEEN 2 PRECEDING AND 2 FOLLOWING)    smoothed_life_years
                           FROM assa_sandbox.covid_actual_and_expected_deaths
                          WHERE clm_month < DATE '2020-03-01'
                       GROUP BY company_code
                               ,clm_month
      ...
      SELECT act.company_code
            ...
            ,exps.total_deaths / exps.life_years                           AS expected_death_rate
            ,exps.smoothed_total_deaths / exps.smoothed_life_years         AS smoothed_total_death_rate
            ,exps.unnatural_deaths / exps.life_years                       AS expected_unnatural_rate
            ,exps.smoothed_unnatural_deaths / exps.smoothed_life_years     AS smoothed_unnatural_death_rate
            ,act.company_code                                              AS service_client_id
        FROM covid_actuals act
             LEFT JOIN monthly_expected_rates exps
                 ON act.company_code = exps.company_code AND act.sex = exps.sex AND act.age_band = exps.age_band AND act.month_of_year = exps.month_of_year
